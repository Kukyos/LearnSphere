    ...
    return (
      <div className={`min-h-screen pt-28 flex items-center justify-center ${theme === 'dark' ? 'bg-brand-950' : 'bg-nature-light'}`}>
        <div className="text-center">
          ...
          <h2 className={`text-2xl font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>Course Not Found</h2>
          <p className={`mb-4 ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>This course is coming soon!</p>
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-brand-950' : 'bg-nature-light'}`}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8">
        ...
        {isEnrolled && (
          <div className={`rounded-2xl p-6 mb-8 ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow-lg`}>
            <div className="flex items-center justify-between mb-3">
              <span className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>
                {isCompleted ? '🎉 Course Completed!' : 'Your Progress'}
              </span>
              <span className={`text-sm font-semibold ${theme === 'dark' ? 'text-brand-200' : 'text-brand-700'}`}>
        ...
        {/* Tabs */}
        <div className={`flex gap-1 p-1 rounded-xl mb-8 ${theme === 'dark' ? 'bg-brand-900' : 'bg-brand-100'}`}>
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-lg text-sm font-bold transition-all capitalize ${
                activeTab === tab
                  ...
                  : theme === 'dark' ? 'text-brand-300 hover:text-white' : 'text-brand-600 hover:text-brand-900'
              }`}
            >
              {tab}
            </button>
        ...
        {activeTab === 'overview' && (
          <div>
            <h2 className={`text-2xl font-bold mb-4 ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>About This Course</h2>
            <p className={`text-lg leading-relaxed mb-8 ${theme === 'dark' ? 'text-brand-200' : 'text-brand-700'}`}>{course.description}</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className={`p-4 rounded-xl text-center ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow`}>
                <BookOpen className="mx-auto mb-2 text-brand-500" size={24} />
                <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>{course.lessons.length}</p>
                <p className={`text-xs ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>Lessons</p>
              </div>
              <div className={`p-4 rounded-xl text-center ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow`}>
                <Star className="mx-auto mb-2 text-yellow-500" size={24} fill="currentColor" />
                <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>{avgRating}</p>
                <p className={`text-xs ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>Rating</p>
              </div>
              <div className={`p-4 rounded-xl text-center ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow`}>
                <Users className="mx-auto mb-2 text-brand-500" size={24} />
                <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>{courseReviews.length}</p>
                <p className={`text-xs ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>Reviews</p>
              </div>
              <div className={`p-4 rounded-xl text-center ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow`}>
                <Clock className="mx-auto mb-2 text-brand-500" size={24} />
                <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>{course.access}</p>
                <p className={`text-xs ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>Access</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'lessons' && (
          <div className="space-y-3">
                ...
                <div
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl transition-all ${canAccess ? 'cursor-pointer' : 'opacity-60'}
                    ${completed
                      ...
                      : theme === 'dark' ? 'bg-brand-900 hover:bg-brand-800' : 'bg-white hover:bg-brand-50'
                    } shadow`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>
                      {index + 1}. {lesson.title}
                    </p>
                    <p className={`text-sm truncate ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>{lesson.description}</p>
                  </div>
            ...
            {!isEnrolled && (
              <div className={`text-center py-8 ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>
        ...
        {activeTab === 'reviews' && (
          <div>
            {/* Submit Review */}
            {isEnrolled && user && (
              <div className={`p-6 rounded-2xl mb-6 ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow`}>
                <h3 className={`font-bold mb-3 ${theme === 'dark' ? 'text-white' : 'text-brand-900'}`}>Write a Review</h3>
                <div className="flex gap-1 mb-3">
                  {[1, 2, 3, 4, 5].map(star => (
                    <button key={star} onClick={() => setReviewRating(star)}>
                      <Star
                        size={24}
                        className={star <= reviewRating ? 'text-yellow-500' : theme === 'dark' ? 'text-brand-700' : 'text-brand-200'}
                ...
                <textarea
                  ...
                  className={`w-full p-3 rounded-xl border resize-none ${
                    theme === 'dark'
                      ? 'bg-brand-800 border-brand-700 text-white placeholder-brand-500'
                      : 'bg-nature-light border-brand-200 text-brand-900 placeholder-brand-400'
            ...
            <div className="space-y-4">
              {courseReviews.map(review => (
                <div key={review.id} className={`p-5 rounded-2xl ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow`}>
                  ...
                  <p className={theme === 'dark' ? 'text-brand-200' : 'text-brand-700'}>{review.text}</p>
                </div>
              ))}
              {courseReviews.length === 0 && (
                <p className={`text-center py-8 ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>No reviews yet. Be the first to review!</p>
              )}
            </div>
          </div>
        )}
