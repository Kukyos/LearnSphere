...
const ProfileDrawer: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const { user, courses, userProgress, theme, logout } = useApp();
  ...
  const getCourseCompletionDate = (courseId: string) => {
    ...
    return progress?.completedDate ? new Date(progress.completedDate).toLocaleDateString() : '';
  };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="text-center">
            <img
              src={user.avatar}
              alt={user.name}
              className="w-24 h-24 rounded-full mx-auto border-4 border-brand-500 mb-4"
            />
            <h3 className={`text-xl font-bold ${theme === 'dark' ? 'text-brand-50' : 'text-brand-900'}`}>
              {user.name}
            </h3>
          ...
          <div className={`rounded-lg p-6 ${theme === 'dark' ? 'bg-brand-900' : 'bg-white'} shadow-lg`}>
            <div className="relative w-40 h-40 mx-auto mb-4">
              ...
              <svg className="transform -rotate-90 w-40 h-40">
                <circle
                  ...
                  cy="80"
                  r="70"
                  stroke={theme === 'dark' ? '#384e2f' : '#c8d4be'}
                  strokeWidth="12"
                  fill="none"
                />
                <circle
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className={`text-3xl font-bold ${theme === 'dark' ? 'text-brand-50' : 'text-brand-900'}`}>
                  {user.points}
                </span>
                <span className={`text-xs ${theme === 'dark' ? 'text-brand-300' : 'text-brand-600'}`}>
                  Points
                </span>
              </div>
            </div>
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className={`text-lg font-bold ${theme === 'dark' ? 'text-brand-50' : 'text-brand-900'}`}>
                  {user.badge}
                </span>
              </div>
              {nextBadge && (
          ...
          {/* Achievements */}
          <div>
            <h4 className={`text-lg font-bold mb-3 ${theme === 'dark' ? 'text-brand-50' : 'text-brand-900'}`}>
              Achievements
            </h4>
              ...
              {badgeLevels.map((badge, index) => (
                ...
                >
                  <Award
                    ...
                    style={{ color: index <= currentBadgeIndex ? badge.color : '#9ca3af' }}
                  />
                  <p className={`text-xs font-semibold ${theme === 'dark' ? 'text-brand-200' : 'text-brand-700'}`}>
                    {badge.name}
                  </p>
                ...
                {completedCourses.map(course => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold truncate ${theme === 'dark' ? 'text-brand-50' : 'text-brand-900'}`}>
                        {course.title}
                      </p>
                      <p className={`text-xs ${theme === 'dark' ? 'text-brand-400' : 'text-brand-500'}`}>
                        Completed: {getCourseCompletionDate(course.id)}
                      </p>
                    </div>
                  </div>
                ))}
