  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-brand-900">Quiz Questions</h3>
        <button
          onClick={addQuestion}
      ...
      {questions.length === 0 && (
        <div className="text-center py-12 border-2 border-dashed border-brand-300 rounded-xl">
          ...
          <p className="text-brand-500">No quiz questions yet. Add questions to create a quiz for this course.</p>
        </div>
      )}

      <div className="space-y-4">
        {questions.map((q, qi) => (
          <div key={q.id} className="border border-brand-200 rounded-xl overflow-hidden bg-white">
            {editingId === q.id ? (
              <div className="p-4 space-y-3">
                ...
                <p className="text-xs text-brand-500">Click the circle to mark the correct answer</p>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setEditingId(null)} className="px-4 py-2 bg-brand-100 text-brand-700 rounded-lg text-sm font-semibold">Cancel</button>
                </div>
              </div>
            ) : (
              <div className="p-4">
                <div className="flex items-start gap-3">
                  <span className="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center text-sm font-bold text-brand-700 flex-none">
                    {qi + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-brand-900 mb-2">{q.question || 'Untitled Question'}</p>
                  ...
                  <div className="flex gap-1 flex-none">
                    <button onClick={() => startEdit(q)} className="p-2 rounded-lg hover:bg-brand-100 text-brand-500 hover:text-brand-600">
                      <Edit3 size={16} />
                    </button>
                    <button onClick={() => deleteQuestion(q.id)} className="p-2 rounded-lg hover:bg-red-50 text-brand-500 hover:text-red-600">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      {questions.length > 0 && rewardRules && (
        <div className="mt-2 p-5 rounded-xl border border-amber-200 bg-amber-50">
          <h4 className="text-base font-bold text-amber-800 mb-1 flex items-center gap-2">
              ...
              <div key={i} className="bg-white rounded-lg p-3 border border-amber-200">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className="w-full px-2 py-1.5 border border-brand-300 rounded-lg bg-white text-brand-900 font-bold text-center text-sm"
                    min="0"
                  />
