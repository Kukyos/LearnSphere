...
export default function PreviewModal({ isOpen, onClose, course }: PreviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl max-h-[85vh] bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-gray-200">
          <h3 className="text-lg font-bold text-gray-900">Course Preview</h3>
        ...
        <div className="overflow-y-auto max-h-[calc(85vh-70px)]">
          ...
          <div className="p-5 space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {course.tags.filter(Boolean).map(tag => (
                  <span key={tag} className="px-2.5 py-1 rounded-full text-xs font-semibold bg-brand-100 text-brand-700">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            {/* Stats */}
            <div className="flex gap-4 py-3 border-t border-b border-gray-200">
            ...
            {course.lessons.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-gray-600 mb-2">Course Content</h4>
                <div className="space-y-2">
                  {course.lessons.map((lesson, i) => (
                    <div key={lesson.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      <span className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center text-xs font-bold text-brand-700">
                        {i + 1}
                      </span>
