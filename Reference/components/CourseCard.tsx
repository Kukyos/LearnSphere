...
const CourseCard: React.FC<CourseCardProps> = ({ course, onPreview, width = "w-[280px]" }) => {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`absolute top-0 left-0 z-50 w-full rounded-lg bg-brand-800 shadow-2xl transition-all duration-300 ease-out origin-center
      ...
      >
        ...
        {/* Expanded Info Area */}
        <div className="p-4 bg-brand-800 rounded-b-lg shadow-2xl ring-1 ring-white/10">
            {/* Actions Bar */}
            <div className="flex items-center gap-3 mb-3">
                ...
                <button className="flex h-8 w-8 items-center justify-center rounded-full border border-brand-400 text-brand-100 hover:border-white hover:text-white transition-colors">
                    <Plus size={16} />
                </button>
                <button className="flex h-8 w-8 items-center justify-center rounded-full border border-brand-400 text-brand-100 hover:border-white hover:text-white transition-colors">
                    <ThumbsUp size={16} />
                ...
                <div className="ml-auto">
                    <button 
                        onClick={() => onPreview(course)}
                        className="flex h-8 w-8 items-center justify-center rounded-full border border-brand-400 text-brand-100 hover:border-white hover:text-white transition-colors"
                    >
                        <ChevronDown size={16} />
                    </button>
                </div>
            ...
            {/* Metadata Line */}
            <div className="flex items-center gap-2 text-xs font-semibold text-brand-50 mb-2">
                <span className="text-brand-300">98% Match</span>
                <span className="border border-brand-400 px-1 rounded text-[10px] text-brand-200 uppercase">{course.difficulty}</span>
                <span className="text-brand-300">{course.duration}</span>
            </div>

            {/* Tags */}
            <div className="flex flex-wrap gap-2 text-[10px] text-brand-100 mb-2">
            ...
            <h4 className="font-bold text-white text-sm mb-1">{course.title}</h4>
