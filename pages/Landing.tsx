    ...
    <div className="min-h-screen bg-nature-light transition-colors duration-300 font-sans overflow-x-hidden">
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="w-full max-w-md overflow-hidden rounded-3xl bg-nature-card shadow-2xl border border-brand-100">
            <div className="relative p-8 text-center">
              <button 
                onClick={() => setShowLoginModal(false)}
                className="absolute right-4 top-4 rounded-full p-2 text-brand-400 hover:bg-brand-100 hover:text-brand-900"
              >
                <X size={20} />
              </button>

              <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-brand-100 text-brand-600 shadow-inner">
                <BookOpen size={32} />
              </div>
              <h2 className="mb-2 text-2xl font-bold text-brand-900">Unlock full access</h2>
              <p className="mb-8 text-sm text-brand-600">Join 250k+ learners. Create a free account to track your progress and save courses.</p>
              
              <div className="space-y-3">
                <button 
                  onClick={() => { setShowLoginModal(false); navigate('/login'); }}
                  className="w-full rounded-xl bg-brand-800 py-3.5 font-bold text-white transition-transform active:scale-95 hover:bg-brand-700"
                >
                  Sign up with Email
                </button>
                <button 
                  onClick={() => { setShowLoginModal(false); navigate('/login'); }}
                  className="w-full rounded-xl border border-brand-200 bg-white py-3.5 font-bold text-brand-700 transition-colors hover:bg-brand-50"
                >
